...
nonlin_form_id_full <- bf(FID ~ exp(logM) * (1 - inv_logit(p)*num_obs/(exp(logd) + num_obs)),
   logM ~ 1 + (1 |t| tamia_id),
   p    ~ 1 + (1 |t| tamia_id),
   logd ~ 1 + (1 |t| tamia_id),
   nl = TRUE,
...
nonlin_prior_full <- c(
  ...
  prior(exponential(1), class = "sd", nlpar = "logd"),
  prior(exponential(4), class = "sd", nlpar = "logM"),
  prior(exponential(2), class = "sd", nlpar = "p"),
  prior(normal(1.5, .5), class = "b", nlpar = "logd"),
  prior(normal(6,.5), class = "b", nlpar = "logM"),
  prior(normal(-1, .2), class = "b", nlpar = "p"),
  prior(gamma(6.25, .25), class = "shape")
  )
